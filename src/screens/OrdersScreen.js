  ...
  return (
    <Screen>
      ...
      <Text style={styles.bodyText}>
        Edit your active cart, place selected items, or review completed orders.
      </Text>

      <Text style={styles.sectionTitle}>Current Cart</Text>
      {items.length === 0 ? (
        <View style={styles.emptyOrderCard}>
          <Text style={styles.cardTitle}>No active cart items</Text>
          <Text style={styles.metaText}>Add food from Home to activate the Orders badge.</Text>
        </View>
      ) : (
        <>
          ...
          {items.map((item) => {
            ...
            return (
              <View style={styles.cartItemCard} key={`${item.id}-order-cart`}>
          ...
          <AppButton title="Open Full Cart" icon="cart-outline" variant="secondary" onPress={openFullCart} />
        </>
      )}

      <Text style={[styles.sectionTitle, styles.sectionSpacing]}>Order History</Text>
      {orders.length === 0 ? (
        <View style={styles.emptyOrderCard}>
          <Text style={styles.cardTitle}>No orders placed yet</Text>
          <Text style={styles.metaText}>Place an order from Cart and it will appear here.</Text>
        </View>
      ) : (
        orders.map((order) => (
